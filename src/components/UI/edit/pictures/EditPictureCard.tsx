...
const EditPictureCard = forwardRef<HTMLDivElement, EditPictureCardProps>(
  (
    ...
    const handleUpdate = async (newTitle: string, newDescription: string): Promise<void> => {
      ...
      try {
        ...
        console.error('Error updating picture:', (error as Error).message)
      }
    }

    const onSave = async (): Promise<void> => {
      const titleInput = document.getElementsByClassName('titleInput')[0] as HTMLInputElement
      const title = titleInput.value
      const descriptionInput = document.getElementsByClassName('descriptionInput')[0] as HTMLInputElement
    ...
    const onDisplayTitle = imgTitle != null ? imgTitle : ''
    const onDisplayDescription = imgDescription != null ? imgDescription : ''
